                    ...
                    <ul slot="dateCellRender" slot-scope="value" class="events">
                        ...
                        <template v-for="(item, key) in list[value.format('YYYY-MM-DD')]">
                            <a-popover className="calendar-pop" :visible="item.visible" title="" trigger="click" :key="item.id" v-if="key <= 3" :getPopupContainer="getPopup">
                                ...
                                <li>
                                    <div @click.stop="showCalendarPop(item)" v-if="key <= 2" style="cursor: pointer;">{{item.title}}</div>
        ...
        getEventsListByCalendar(value) {
            ...
            getEventsListByCalendar(obj).then(res => {
                if (res && res.data && res.data.list) {
                    ...
                    Object.keys(listData).forEach(dateKey => {
                        listData[dateKey].forEach(item => {
                            // 使用 $set 确保响应式
                            app.$set(item, 'visible', false);
                            app.$set(item, 'visibleMore', false);
                            app.$set(item, 'visibleInner', false);
                            // 计算 myStatus 和 waitConfirm
                            if (item.memberList && item.memberList.length > 0) {
                                const myMember = item.memberList.find(m => m.member_code === app.currentMemberCode);
                                if (myMember) {
                                    app.$set(item, 'myStatus', myMember.status);
                                    app.$set(item, 'waitConfirm', myMember.status === 0 && item.created_by !== app.currentMemberCode ? 1 : 0);
                                }
                            }
                        });
                    });
        ...
        getMemberList() {
            _allList(this.requestData).then(res => {
                if (res && res.data && res.data.list) {
                    ...
                    this.memberListTemp = res.data.list;
                } else {
                    this.memberList = [];
                    this.memberListTemp = [];
                }
        ...
        showCalendarPop(record) {
            ...
            _.forIn(this.list, (calendarList, dateKey) => {
                calendarList.forEach((item, index) => {
                    this.$set(this.list[dateKey][index], 'visible', false);
                    this.$set(this.list[dateKey][index], 'visibleMore', false);
                    this.$set(this.list[dateKey][index], 'visibleInner', false);
                });
            });
            if (record) {
                // 找到对应的日程项并设置 visible
        ...
        showCalendarPopInner(record) {
            ...
            if (record) {
                _.forIn(this.list, (calendarList, dateKey) => {
                    const index = calendarList.findIndex(item => item.code === record.code);
                    if (index !== -1) {
                        this.$set(this.list[dateKey][index], 'visibleInner', true);
                    }
                });
            }
        },
        showCalendarPopMore(record) {
            ...
            if (record) {
                _.forIn(this.list, (calendarList, dateKey) => {
                    const index = calendarList.findIndex(item => item.code === record.code);
                    if (index !== -1) {
                        this.$set(this.list[dateKey][index], 'visibleMore', true);
                    }
                });
            }
        },
                ...
                app.$confirm({
                    ...
                    onOk() {
                        del({eventsCode: record.code}).then(res=>{
                            ...
                            // 刷新日历数据
                            app.getEventsListByCalendar(app.calendarValue);
                        })
                    }
                });
            }
