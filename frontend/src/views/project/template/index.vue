    ...
    <div class="project-template-index">
        <wrapper-content :showHeader="false">
            ...
            >
                ...
                <a-list-item slot="renderItem" slot-scope="item,index">
                    <span slot="actions" @click="doAction(item,'stages')">
                         <a-tooltip :title="item.is_system ? '查看任务流' : '任务列表'">
                              <a-icon type="bars"/>
                         </a-tooltip>
                    </span>
        ...
        >
            ...
            >
                ...
                <a-form-item>
                    <div class="task-stages-section">
                        <div class="section-header">
                            <span class="section-title">任务流（任务阶段）</span>
                            <a-button type="dashed" size="small" icon="plus" @click="addStage">添加阶段</a-button>
                        </div>
                        <div class="stage-list" v-if="taskStages.length">
                            <div class="stage-item" v-for="(stage, idx) in taskStages" :key="idx">
                                <a-input
                                    size="small"
                                    v-model="stage.name"
                        ...
                        <div class="stage-empty muted" v-else>
                            暂未添加任务阶段，保存后可在任务列表中管理
                        </div>
                    </div>
                </a-form-item>
                <a-form-item
...
<script>
    ...
    import {doData as saveStage} from '@/api/taskStagesTemplate';
        ...
        methods: {
            init(reset = true) {
                ...
                list(app.requestData).then(res => {
                    ...
                    app.dataSource = app.dataSource.concat(listData);
                    app.pagination.total = res.data ? res.data.total : 0;
            ...
            addStage() {
                this.taskStages.push({name: ''});
            },
            removeStage(index) {
                this.taskStages.splice(index, 1);
            },
                ...
                if (action == 'edit' || action == 'new') {
                    ...
                    if (action == 'edit') {
                        ...
                        setTimeout(function () {
                            ...
                            app.newData = record;
                            // 编辑时加载已有的任务阶段
                            if (record.task_stages && record.task_stages.length) {
                                app.taskStages = record.task_stages.map(s => ({name: s.name}));
                            }
                        }, 0);
                    } else {
                        // 新建时默认添加3个常用阶段
            ...
            handleOk() {
                ...
                doData(obj).then(res => {
                    ...
                    // 保存模板成功后，创建任务阶段
                    let templateCode = res.data.code || (app.newData.code);
                    let stages = app.taskStages.filter(s => s.name && s.name.trim());
                    if (stages.length && templateCode && !app.newData.code) {
                        // 仅新建时自动创建任务阶段（编辑模式阶段通过任务列表页管理）
                        let promises = stages.map(s => {
                            return saveStage({
                                name: s.name.trim(),
