	...
	<div class="project-space-task" :class="`${project.task_board_theme} ${viewType}`">
            ...
            <section class="nav-body">
                <ul class="nav-wrapper nav nav-underscore pull-left">
                    ...
                    <li><a class="app" data-app="gantt"
                           @click="$router.push('/project/space/gantt/' + code)">
                        甘特图</a>
                    </li>
                </ul>
            </section>
        ...
        <wrapper-content :showHeader="false">
            <draggable v-show="viewType == 'task-board'" v-model="taskStages"
                       ...
                       :drag-class="'stage-drag'"
                       :animation="250"
                       :force-fallback="false"
                       :delay="100"
                       :delay-on-touch-only="true"
                       id="board-scrum-stages" class="board-scrum-stages" @end="stageSort" @start="stageDragStart"
                       v-dragscroll2>
                <div class="scrum-stage" v-for="(stage,index) in taskStages" :key="index" :id="stage.code"
                    ...
                    <!--<a-tooltip placement="top" >
                     <template slot="title">
                         {{ stage.name }}
                         <span class="task-count" v-if="stage.list && stage.list.length > 0"> · {{ stage.list.length }}</span>
                     </template>-->
                    <header class="scrum-stage-header ui-sortable-handle" v-show="!stage.tasksLoading">
                        <div class="stage-name hinted">
                            {{ stage.name }}
                            <span class="task-count"
                                  v-if="stage.tasks && stage.tasks.length > 0"> · {{ stage.tasks.length }}</span>
                        </div>
                            ...
                            <section :id="stage.code" :task-type-index="index"
                                ...
                                <a-spin wrapperClassName="tasks-loading" :spinning="stage.tasksLoading">
                                    ...
                                    <draggable v-model="stage.unDoneTasks"
                                               ...
                                               :delay-on-touch-only="true"
                                               :animation="200"
                                               @end="taskSort" @start="taskDragStart"
                                            ...
                                            <div class="handler-wrap">
                                                <a-dropdown :trigger="['click']">
                                                    <a class="executor-handler"><img
                                                            :src="defaultExecutor && defaultExecutor.avatar"
                                                            class="avatar img-circle img-24 hinted"> <span
                                                            class="creator-handler-text name">{{ defaultExecutor && defaultExecutor.name }}</span></a>
                                                    <a-menu class="executor-handler-menu" @click="selectExecutor"
                                                        ...
                                                        <a-menu-item
                                                            ...
                                                            <a-icon type="check" class="muted"
                                                                    v-if="defaultExecutor && member.code == defaultExecutor.code"></a-icon>
                                                        </a-menu-item>
                                                    </a-menu>
                                                </a-dropdown>
                                            </div>
    ...
    export default {
                ...
                if (from.name == 'taskdetail') {
                    ...
                    if (stageIndex != undefined) {
                        ...
                        getTasks(params).then((res) => {
                            ...
                            this.$set(this.taskStages[stageIndex], 'doneTasks', doneTasks);
                            this.$set(this.taskStages[stageIndex], 'unDoneTasks', unDoneTasks);
                        });
                    }
                }
            },
        ...
        methods: {
            ...
            getTasks(stage, index, showLoading) {
                ...
                params = Object.assign(params, searchParams);
                // 初始化动态属性，确保 Vue 2 响应式
                this.$set(stage, 'doneTasks', []);
                this.$set(stage, 'unDoneTasks', []);
                getTasks(params).then((res) => {
                    let canNotReadCount = 0;
                    res.data.forEach((task) => {
                        if (!task.canRead) {
                    ...
                    this.$set(stage, 'canNotReadCount', canNotReadCount);
            ...
            showTaskCard(index = false, show = true) {
                this.taskStages.forEach((v, i) => {
                    this.$set(this.taskStages[i], 'showTaskCard', false);
                });
                if (index === false) {
                    return false;
                }
                this.$set(this.taskStages[index], 'showTaskCard', show);
                this.$nextTick(() => {
                    //滚动创建到创建窗口
                    const stageRef = this.$refs[index + '-stage'];
                    const inputRef = this.$refs[`inputTaskName${index}`];
                    if (stageRef && stageRef[0]) {
                        stageRef[0].scrollIntoView('#card' + index);
                    }
            ...
            createTask(stageCode, stageIndex) {
                ...
                if (this.defaultExecutor && this.defaultExecutor.code) {
                ...
                switch (actionKey) {
                    ...
                    case 'recycleBatch':
                        ...
                        this.$confirm({
                            ...
                            cancelText: `再想想`,
                            onOk() {
                                const taskCodes = app.taskStages[stageIndex].tasks.filter(task => task.canRead).map(task => task.code);
                                if (!taskCodes.length) {
                                    app.$message.info('当前列表没有可回收任务');
                                    return Promise.resolve();
                                }
                                recycleBatch({taskCodes: JSON.stringify(taskCodes)}).then(res => {
                                    const result = checkResponse(res);
                                    if (!result) {
                                        return false;
                                    }
                                    const hiddenTasks = app.taskStages[stageIndex].tasks.filter(task => !task.canRead);
                                    app.taskStages[stageIndex].tasks = hiddenTasks;
                                    app.taskStages[stageIndex].canNotReadCount = hiddenTasks.length;
            ...
            creteStage() {
                ...
                createState({name: this.stageName, projectCode: this.code}).then(res => {
                    ...
                    this.$set(stage, 'showTaskCard', false);
            ...
            taskSort(event) {
                ...
                // 添加完成动画类
                event.item.classList.add('drop-complete');
                setTimeout(() => {
                    event.item.classList.remove('drop-complete');
                }, 300);
            },
            taskDragStart(event) {
                // 添加拖拽类以应用全局样式
...
<style lang="less">
    ...
    @import "../../../assets/css/components/task-enhanced";
