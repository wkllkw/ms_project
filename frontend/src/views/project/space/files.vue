...
<script>
    ...
    export default {
        ...
        watch: {
            uploader: {
                handler(newVal, oldVal) {
                    //监听是否有上传文件行为
                    if (!newVal || !newVal.fileList) return;
        ...
        mounted() {
            ...
            // 监听文件上传完成事件，刷新文件列表
            this.$root.$on('fileUploadComplete', this.onFileUploadComplete);
        },
        beforeDestroy() {
            this.$root.$off('fileUploadComplete', this.onFileUploadComplete);
        },
        methods: {
            onFileUploadComplete(tempData) {
                // 上传完成后，如果上传的目标项目是当前项目，则刷新文件列表
                if (tempData && tempData.projectCode === this.code) {
                    setTimeout(() => {
                        this.getFiles();
                    }, 500);
                }
            ...
            getFiles(reset = true) {
                ...
                list(app.requestData).then(res => {
                    ...
                    if (res.data && res.data.list) {
                        ...
                        app.pagination.total = res.data.total || 0;
