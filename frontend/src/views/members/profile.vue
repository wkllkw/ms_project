...
<script>
    ...
    export default {
        ...
        computed: {
            homePath() {
                const currentOrganization = this.$store.state.currentOrganization || getStore('currentOrganization', true);
                if (currentOrganization && currentOrganization.code) {
                    return `${config.HOME_PAGE}/${currentOrganization.code}`;
                }
                return '/';
            },
        },
