    ...
    <div class="account-index">
        ...
        >
            <div v-if="currentUser" style="margin: 0 0 24px 0;">
                <!-- 当前用户信息 -->
                <div style="margin-bottom: 16px; padding: 12px 16px; background: #fafafa; border-radius: 6px; display: flex; align-items: center;">
                    <a-avatar :src="currentUser.avatar" :size="40" icon="user" style="margin-right: 12px; flex-shrink: 0;"/>
                    <div>
                        <div style="font-weight: 500; font-size: 14px; color: rgba(0,0,0,0.85);">{{ currentUser.name }}</div>
                        <div style="font-size: 12px; color: rgba(0,0,0,0.45);">{{ currentUser.mobile || currentUser.email || '暂无联系方式' }}</div>
                    </div>
                </div>
                <!-- 权限角色选择 -->
                <template v-if="authList && authList.length > 0">
                    <div style="margin-bottom: 8px; font-size: 13px; color: rgba(0,0,0,0.65);">请选择要分配的权限角色：</div>
                    <div style="margin-bottom: 24px;">
                        <a-radio-group v-model="currentUser.authorize" buttonStyle="solid" style="width: 100%;">
                            <a-radio-button :value="auth.value" v-for="auth in authList" :key="auth.value" style="margin-bottom: 8px;">
                                {{auth.label}}
                            </a-radio-button>
                        </a-radio-group>
                    </div>
                ...
                <!-- 空状态：没有权限角色时的提示 -->
                <template v-else>
                    <a-empty description="暂无可分配的权限角色" style="padding: 24px 0;">
                        <a-button type="primary" @click="goAuthManage">
                            <a-icon type="plus"/> 前往创建权限角色
                        </a-button>
                    </a-empty>
                    <a-alert
                            type="info"
                    ...
                    >
                        <template slot="message">
                            请先在 <a @click="goAuthManage" style="font-weight:500;">系统管理 &gt; 权限管理</a> 中创建权限角色（如"管理员"、"项目经理"、"普通成员"等），然后再回来为用户分配权限。
                        </template>
                    </a-alert>
                    <div class="action-btn" style="text-align: right;">
                        <a-button class="middle-btn" @click="authInfo.modalStatus = false">关闭
                        </a-button>
                    </div>
                </template>
            </div>
        ...
        methods: {
            ...
            init(){
                ...
                list(app.requestData).then(res=>{
                    ...
                    app.authList = [];
                    let authData = res.data.authList || [];
                    authData.forEach(function (v) {
                        const obj = {
                            label: v.title,
                            value: v.id
                        };
            ...
            authApply() {
                let app = this;
                if (!app.currentUser.authorize) {
                    app.$message.warning('请先选择一个权限角色');
                    return;
                }
            ...
            goAuthManage() {
                this.authInfo.modalStatus = false;
